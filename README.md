...
![Theme preview](https://lexingtonthemes.com/OpenGraph/riflesso/twitter.png)


## Links
- **Theme specs:** https://lexingtonthemes.com/templates/riflesso  
- **Documentation:** https://lexingtonthemes.com/documentation  
- **Changelog:** https://lexingtonthemes.com/changelog/riflesso  
...
**Full reset and seed (one document per content type):** To clear all existing documents and re-seed from the content folder so that Sanity Studio ends up with exactly one document per content type (post, teamMember, gallery, product, legalPage), run from the repo root:

```bash
SANITY_TOKEN=your-token-here pnpm run seed:all
```

This deletes all documents of each type (in batches), removes any documents with the same IDs that will be created (to avoid `_type` conflicts), waits 3 seconds, then creates one document per markdown file in each collection.
